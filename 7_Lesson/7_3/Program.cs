int[,] CreateEmpty2DArray(int rowsNumber, int columnsNamber)
{
    return new int[rowsNumber, columnsNamber];
}

void Fill2DArray(int[,] array2D, int minBorderForRandom, int maxBorderForRandom)
{
    int rowsNumber = array2D.GetLength(0);
    int columnsNamber = array2D.GetLength(1);

    for (int row_index = 0; row_index < rowsNumber; row_index++)
    {
        for (int column_index = 0; column_index < columnsNamber; column_index++)
        {
